<template>
  <div class="project-page">
    ...
    <section class="container mx-auto pt-8">
      ...
      <hr />
      <div class="block mb-2" v-for="campaign in campaigns.data" :key="campaign.id">
        <div class="w-full lg:max-w-full lg:flex mb-4">
          <div
            class="h-48 lg:h-auto lg:w-48 flex-none bg-cover rounded-t lg:rounded-t-none lg:rounded-l text-center overflow-hidden"
            :style="
                  'background-color: #bbb; background-position: center; background-image: url(\''+ $axios.defaults.baseURL + '/../../' + campaign.image_url+ '\')'
            "
          ></div>
          <nuxt-link
          ...
          >
            <div class="mb-8">
              ...
              <p class="text-sm text-gray-600 flex items-center mb-2">
                <!-- Rp. {{ new Intl.NumberFormat().format(campaign.goal_amount) }} &middot; {{ transaction.created_at  }} &middot; {{ transaction.status }} -->
              </p>
...
export default {
  ...
  async asyncData({$axios, app}){
    const  campaigns = await $axios.$get('/campaigns?user_id=' + app.$auth.user.id)
