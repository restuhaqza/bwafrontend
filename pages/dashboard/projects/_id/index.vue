          ...
          >
            <div>
              ...
              <p class="text-sm text-gray-600 flex items-center mb-2">
                {{ new Intl.NumberFormat().format(transaction.amount) }} &middot; {{ transaction.created_at }}
              </p>
            </div>
          </div>
        </div>
...
export default {
  ...
  async asyncData({$axios, params}){
    const campaign = await $axios.$get('/campaigns/' + params.id)
    const transactions = await $axios.$get('/campaign/' + params.id + '/transactions')
  ...
  methods: {
    ...
    async load(){
      const campaign = await this.$axios.$get('/campaigns/' + this.$route.params.id)
      const transactions = await this.$axios.$get('/campaign/' + this.$route.params.id + '/transactions')
    ...
    async upload(file){
      ...
      try {
        let response = await this.$axios.post('/campaign-images', formData, {
          headers: {
